...
trainer_stats_name="codecarbon_500ms"
...
class SimpleFileOutput(BaseOutput): 
    ...
    def to_csv(self, total: EmissionsData, delta: EmissionsData):
        ...
        new_df = pd.DataFrame.from_records([dict(delta.values)])
    ...
    def live_out(self, total: EmissionsData, delta: EmissionsData):
        row = dict(delta.values)
        row["timestamp"] = time.perf_counter() - self.start_time
        current_step = self.iteration_ref.iteration
...
class CodeCarbonStats(base.TrainerStats):
    ...
    def start_train(self) -> None:
        ...
        for handler in self.total_training_tracker._output_handlers:
            handler.start_time = self.start_time
            handler.iteration_ref = self
